...
const UpcomingAppointmentsCard: React.FC<UpcomingAppointmentsProps> = ({
  patientUuid,
  setUpcomingAppointment,
}) => {
  const { t } = useTranslation();
  const startDate = dayjs(new Date().toISOString())
    .subtract(6, "month")
    .toISOString();
  ...
  const {
    ...
    isError,
    isLoading,
  } = useAppointments(patientUuid, startDate, ac);

  const todaysAppointments = appointmentsData?.todaysAppointments?.length
    ? appointmentsData?.todaysAppointments
    : [];
    ...
    const structuredListBodyRowGenerator = () => {
      return upcomingAppointment.map((appointment, i) => (
        <StructuredListRow
          label
          key={`row-${i}`}
          className={styles.structuredList}
        >
          <StructuredListCell>
